...
const CreateExamdetails2 = () => {
  ...
  const apiUrl = apiConfig.getResourceUrl("examdetails2");
  const metadataUrl = apiConfig.getResourceMetaDataUrl("Examdetails2");
  ...
  // ✅ Automatically set applicantid from sessionStorage
  // useEffect(() => {
  //   const id = sessionStorage.getItem("applicantid");
  //   if (id) {
  //     setDataToSave((prev: Record<string, any>) => ({
  //       ...prev,
  //       applicantid: id,
  //     }));
  //   } else {
  //     console.warn("No applicantid found in sessionStorage.");
  //   }
  // }, []);
    useEffect(() => {
      setDataToSave((prev: any) => ({
        ...prev,
        applicantid: "e3c73232-6d68-4854-97d7-0e12f8127824-24",
      }));
    }, []);
  ...
  const handleCreate = async () => {
    if (!dataToSave.applicantid) {
      alert("Error: Applicant ID is missing. Cannot submit form.");
      return;
    }
  ...
  return (
    <div>
      <div>
        <h2 className='mt-5 fs-4'>JEE Advanced (Joint Entrance Examination - 2025)</h2>
        <hr />
      </div>
...
export default CreateExamdetails2;

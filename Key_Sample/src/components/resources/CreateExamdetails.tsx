...
const CreateExamdetails = () => {
    ...
    const apiUrl = apiConfig.getResourceUrl("examdetails");
    const metadataUrl = apiConfig.getResourceMetaDataUrl("Examdetails");
    ...
    const { data: metaData, isLoading, error } = useQuery({
        queryKey: ['resMetaData', 'Examdetails'],
        queryFn: async () => {
    ...
    return (
        <div>
            <div>
                <h2 className='mt-5 fs-4'>JEE Main (Joint Entrance Examination - 2025)</h2>
                <hr />
            </div>

...
const ReadEducationdetails2 = () => {
    ...
    const { data: dataRes, isLoading: isLoadingDataRes, error: errorDataRes } = useQuery({
        queryKey: ['resourceData', 'educationdetails2', userId],
        queryFn: async () => {
            ...
            const response = await fetch(
                `${apiConfig.getResourceUrl('educationdetails2')}?` + params.toString(),
                {
                    method: "GET",
    ...
    const { data: dataResMeta, isLoading: isLoadingDataResMeta, error: errorDataResMeta } = useQuery({
        queryKey: ['resourceMetaData', 'Educationdetails2'],
        queryFn: async () => {
            const response = await fetch(
                `${apiConfig.getResourceMetaDataUrl('Educationdetails2')}?`,
                {
                    method: "GET",
    ...
    if (!userId) {
        return (
            <div>
                <div><h2> ReadEducationdetails2 </h2></div>
...
export default ReadEducationdetails2;
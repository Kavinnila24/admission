...
const CreatePermaddress = () => {
    ...
    const { data: metaData, isLoading, error } = useQuery({
        queryKey: ['resMetaData', 'Permaddress'],
        queryFn: async () => {
    ...
    return (
        <div>
            <div className="d-flex gap-2 mt-4">
                <input type="checkbox" id="sameAddress" checked={sameAsCurrent} onChange={handleCheckboxChange} className="form-check-input" style={{ width: "20px", height: "20px" }} />
                <label htmlFor="sameAddress" className="fs-5">Permanent Address same as Current Address</label>
            </div>
            <div className="container mt-3">
                                    ...
                                    {field.foreign ? (
                                        <>
                                            ...
                                            <div className="dropdown">
                                                <button className="btn btn-secondary dropdown-toggle w-100" type="button" data-bs-toggle="dropdown" disabled={sameAsCurrent}>
                                        ...
                                        <>
                                            <label>{field.required && <span style={{ color: "red" }}>*</span>} {customFieldLabels[field.name] || field.name}</label>
                                            <select name={field.name} required={field.required} value={dataToSave[field.name] || ""} disabled={sameAsCurrent} onChange={(e) => setDataToSave({ ...dataToSave, [e.target.name]: e.target.value })} className="form-control custom-input">
                                                <option value="">Select {field.name}</option>
                                                {enums[field.possible_value]?.map((enumValue, idx) => (<option key={idx} value={enumValue}>{enumValue}</option>))}
                                            </select>
                                        </>
                                    ) : (
                                        <>
                                            <label>{field.required && <span style={{ color: "red" }}>*</span>} {customFieldLabels[field.name] || field.name}</label>
                                            <input type={field.type} name={field.name} required={field.required} placeholder={customFieldLabels[field.name] || field.name} value={dataToSave[field.name] || ""} disabled={sameAsCurrent} onChange={(e) => setDataToSave({ ...dataToSave, [e.target.name]: e.target.value })} className="form-control custom-input" />
                                        </>
                                    )}
                                </div>
                            );
                ...
                <div className="d-flex justify-content-end">
                    <button id="save_button" className="btn btn-success mt-2" onClick={handleCreate}>Save</button>
                </div>
            </div>
            {showToast && (

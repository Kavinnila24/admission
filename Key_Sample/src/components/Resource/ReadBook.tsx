import React, { useState } from 'react';
  ...
  const apiUrl = `${apiConfig.getResourceUrl('book')}?`
  const metadataUrl = `${apiConfig.getResourceMetaDataUrl('Book')}?`
  const BaseUrl = '${apiConfig.API_BASE_URL}';
  // Fetch resource data
  
  const {data:dataRes,isLoading:isLoadingDataRes,error:errorDataRes}= useQuery({
    queryKey: ['resourceData', 'book'],
     queryFn: async () => {
...
const accessToken = getCookie("access_token");
  ...
  if (!accessToken) {
    throw new Error("Access token not found");
      ...
      const response = await fetch(
        `${apiConfig.getResourceUrl('book')}?` + params.toString(),
        {
          method: "GET",
          headers: {
  ...
  const {data: dataResMeta,isLoading:isLoadingDataResMeta,error:errorDataResMeta} = useQuery({
    queryKey: ['resourceMetaData', 'book'],
   queryFn: async () => {
      const response = await fetch(
        `${apiConfig.getResourceMetaDataUrl('book')}?`,
        {
          method: "GET",

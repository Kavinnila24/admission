import React, { useState } from 'react';
  ...
  const apiUrl = `${apiConfig.getResourceUrl('author')}?`
  const metadataUrl = `${apiConfig.getResourceMetaDataUrl('Author')}?`
  const BaseUrl = '${apiConfig.API_BASE_URL}';
  // Fetch resource data
  
  const {data:dataRes,isLoading:isLoadingDataRes,error:errorDataRes}= useQuery({
    queryKey: ['resourceData', 'author'],
     queryFn: async () => {
...
const accessToken = getCookie("access_token");
  ...
  if (!accessToken) {
    throw new Error("Access token not found");
      ...
      const response = await fetch(
        `${apiConfig.getResourceUrl('author')}?` + params.toString(),
        {
          method: "GET",
          headers: {
  ...
  const {data: dataResMeta,isLoading:isLoadingDataResMeta,error:errorDataResMeta} = useQuery({
    queryKey: ['resourceMetaData', 'author'],
   queryFn: async () => {
      const response = await fetch(
        `${apiConfig.getResourceMetaDataUrl('author')}?`,
        {
          method: "GET",
